...
class InventoryController extends Controller
{
  use InventoryTrait;
    ...
    {
        $inventories = Inventory::where('deleted', 0)->with('item','branch')->orderBy('id', 'desc')->simplePaginate(10);
        $branches = Branch::all();
    ...
    {   
        ...
        if($newInventory){
            ...
            $this->SaveTrackInventory($newInventory->id,$newInventory->item_id,auth()->user()->id,$newInventory->branch_id,$newInventory->sku,$newInventory->qty,'Manual Added');
            if(!$auto_bar_code){
                //update sku counter 
    ...
    {
        $inventories = Inventory::where('deleted', 0)->with('item','branch')->get();
         $inventoryData =[]; 
         $i=1;       
    ...
    function massDistribute(){
     ...
     function searchSku($query){
       ...
       $items = Inventory::where('deleted', 0)->where('sku', 'LIKE', '%'.$query.'%')->get();
        $str = '';
        if($items->count()){ 
    ...
    function getInventoryById($id){
        $inventories = Inventory::where('deleted', 0)->where('id',$id)->with('item','branch')->get();
         $inventoryData =[]; 
         $html = ''; 
         ...
         foreach($inventories as $inventory){
            ...
            $up = '<span  data-toggle="modal" data-target="#updateModal" data-inv="'.$inventory->id .'" data-whatever="'. $inventory->sku .'" data-qty="'.$inventory->qty .'" class="btn mr-2 btn-default btn-sm btn-item-table" title="Add/Remove Qty"  data-location="'. url('HelperController/getCSRF') .'"><i class="fas fa-plus"></i> / <i class="fas fa-minus"></i></span>';
            ...
            $html .="<td><div class='btn-group'>".$bc.$up.$tr.$edt.$del."</div></td>";                  
    ...
    function deleteSkuInventory($sku, $option){
        $items = Inventory::where('deleted', 0)->where('sku', $sku)->get();
        
        if(count($items)){
            foreach($items as $item){
                Inventory::where('id', $item->id)->update(['deleted'=>1]);
            }
            return ['success'=> true,'msg'=>''];
        }        

        return ['success'=> false, 'msg'=>'Could not find the SKU !'];
