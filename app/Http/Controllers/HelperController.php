...
class HelperController extends Controller
{
	...
	function printBarcode(Request $request){
		...
		$pdf= false;
		 if($data['configs']->br_line == '1'){
			$page = 'admin.singlelinebarcodepdf';
			//$page = 'admin.singlelinebarcode';
			$pdf= true;
		} elseif($data['configs']->br_line == '2') {
			$page = 'admin.twolinebarcode';
		} elseif($data['configs']->br_line == '3') {
			$page = 'admin.old_style_two_column_barcode';
			$pdf= true;
		} else{
	...
	function updateInventoryQty(Request $request){
		...
		$status = $this->updateInventoryCustom($validatedData['data_inv'], $validatedData['qty'], $validatedData['action']);
